...
check("post_l = 30 (accounted = gate here; no backfill rows seeded)", abs(c["post_l"] - 30.0) < 1e-9)
check("leak = 30 NET (provider 60 − accounted 30; d3 billed, in neither gate nor backfill)", abs(c["leak"] - 30.0) < 1e-9)
check("coverage = 50%", abs(c["coverage"] - 50.0) < 1e-9)

print("-- leak_line: flags the NET shortfall + points to reconcile (not 'install the gate') --")
line = LS.leak_line(since=SINCE)
check("leak_line returns a string", isinstance(line, str))
check("leak_line flags the shortfall (behind provider) + points to reconcile", "behind provider" in line and "reconcile" in line)
check("leak_line shows the ~$30 figure", "$30.00" in line)
check("leak_line does NOT misframe it as 'install the gate'", "install" not in line)
...
check("no-leak line reports % accounted", clean is not None and "accounted" in clean)
check("no-leak line is not a shortfall alert", "behind provider" not in clean)
...
# Even if an admin oracle is reachable AND the env flag is set, the MAIN reconcile path must NOT consult it or record
# its output. Admin keys only cross-check in a SEPARATE dev tool; they are never a production reconcile input. (The
# admin-oracle-into-ledger path was deleted from reconcile_realtime — realtime is reconstructed agentically from the
# conversations instead, exactly like batch + GPU.)
RTO.by_project_day = lambda since: (
    [{"project": "lmm", "provider": "openai", "day": "2026-06-05", "cost": 900.0}],
    {"ours_total": 1000.0, "other_org": 50.0, "by_org": {"Healiom": 900.0}})
...
check("admin-oracle path REMOVED: reconcile_realtime never reports source=admin-oracle", ro_r.get("source") != "admin-oracle")
_oracle_rows = [row for row in budget.by_dims() if row.get("model") == "(realtime-oracle)"]
check("admin NEVER writes the ledger: 0 (realtime-oracle) rows even with SPENDGUARD_ADMIN_ORACLE set", len(_oracle_rows) == 0)
