...
def test_rollup_maps_stamps_and_filters_by_project(monkeypatch):
    raw = [
        dict(day="2026-06-15", provider="openai", model="gpt-5.5", kind="batch", project="lmm", cost=2.5, calls=4),
        dict(day="2026-06-15", provider="anthropic", model="claude-opus-4-8", kind="realtime", project="lmm", cost=1.0, calls=2),
        dict(day="2026-06-14", provider="openai", model="gpt-5.5", kind="meta", project="llmseg", cost=0.10, calls=1),
        dict(day="2026-06-14", provider=None, model=None, kind=None, project="manga2anime", cost=0.000001, calls=1),
    ]
    monkeypatch.setattr(budget, "by_dims", lambda since=None: raw)
    # owns project=lmm → lmm rows + llmseg (spendguard's own meta always rides along); manga2anime excluded
    monkeypatch.setattr(saas, "conn", lambda: {"contributor": "Ada@Healiom.com", "project": "lmm"})

    rows = saas._rollup_rows()
    assert {r["project"] for r in rows} == {"lmm", "llmseg"}     # not manga2anime
    assert all(r["member_ref"] == "ada@healiom.com" for r in rows)
    by = {(r["project"], r["model"], r["channel"]): r for r in rows}
    assert by[("lmm", "gpt-5.5", "batch")]["spend_micros"] == 2_500_000 and by[("lmm", "gpt-5.5", "batch")]["calls"] == 4
    assert by[("lmm", "claude-opus-4-8", "realtime")]["kind"] == "workload"
...
def test_rollup_no_filter_sends_all_projects(monkeypatch):
    raw = [
        dict(day="2026-06-15", provider="openai", model="m", kind="batch", project="lmm", cost=1.0, calls=1),
    ...
    rows = saas._rollup_rows()
    assert len(rows) == 2 and {r["project"] for r in rows} == {"lmm", "llmseg"}
