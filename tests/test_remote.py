...
# Remote/GPU spend is its OWN category now (remote_compute_usd, its own cap) — NOT the LLM total. So it is
# verified through remote_dec, not spent_since (which is LLM batch+realtime only). This is the category split.
def remote_spent(project):
    return float(budget._ledger().remote_dec(since=TODAY, where={"project_primary": project}))
...
ck("ingest_remote: scoped to the project (actual-$, own GPU cap)", abs(remote_spent("manga2anime") - 4.0) < 1e-9)
ck("ingest_remote: does NOT leak into another project", remote_spent("other") == 0.0)
ck("ingest_remote: GPU spend is NOT in the LLM total (spent_since)", budget.spent_since(TODAY, project="manga2anime") == 0.0)
# re-ingest the SAME box → replaces, never doubles
budget.ingest_remote("m2a-h200", "manga2anime", rows)
ck("ingest_remote: IDEMPOTENT (re-sync replaces, no double-count)", abs(remote_spent("manga2anime") - 4.0) < 1e-9)
...
ck("ingest_remote: different box accumulates", abs(remote_spent("manga2anime") - 5.0) < 1e-9)
...
ck("sync: re-ingested under the box label (replaced the earlier 4.0)",
   abs(remote_spent("manga2anime") - (3.0 + 1.0)) < 1e-9)  # m2a-h200 now 3.0, m2a-3090 still 1.0
