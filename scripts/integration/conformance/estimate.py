...
def render(est):
    """Human-readable projection for `--estimate`."""
    out = ["Conformance soak — ZERO-SPEND estimate (no calls made):", ""]
    out.append("  %-4s %-34s %-8s %8s %12s %10s" % ("id", "behaviour", "spend", "calls", "per-call $", "total $"))
    for r in est["rows"]:
        pc = "—" if r["per_call_usd"] is None else ("$%.5f" % r["per_call_usd"])
        tot = "—(unpriced)" if r["usd"] is None else ("$0" if r["usd"] == 0 else "$%.2f" % r["usd"])
        out.append("  %-4s %-34s %-8s %8d %12s %10s" % (r["id"], r["title"][:34], r["spend_class"], r["calls"], pc, tot))
    out.append("")
