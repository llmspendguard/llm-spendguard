...
def _ensure_insights_schema(c):
    """Create the insights table (+ living-insight migration columns), graph_nodes / graph_edges (+ indexes), and
    seg_attribution (+ migration + index). Idempotent; run once per pooled connection by config.pooled_ledger_conn.
    Other modules (history, models, conv) add THEIR OWN tables on this same connection where they use it."""
...
def _insights_db():
    """The insights / graph / seg_attribution tables, on the shared pooled ledger connection (config.pooled_ledger_conn,
    keyed 'learn') — reused, tuned, fork-safe. `_lock` still serializes the module's read-modify-write sites."""
    return config.pooled_ledger_conn("learn", _ensure_insights_schema)
