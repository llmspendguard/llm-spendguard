...
## The four MCP tools (`spendguard mcp`)
...
Register the command `spendguard mcp` as a stdio MCP server. It must run under the **gated** interpreter (the
spending tools `require()` the gate and fail closed otherwise); the read-only tools (`advise`, `models`) do not
spend and need no gate.
...
| claim | code |
|---|---|
| four tools, these names/schemas | `mcp_server._TOOLS` |
...
Guards: `tests/test_mcp_server.py` (all four tools + the protocol frame), `tests/test_advise.py`,
